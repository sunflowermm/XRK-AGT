...
export default class Bot extends EventEmitter {
  ...
  async _createHttpsProxyServer() {
    ...
    if (tlsConfig.http2 === true) {
      try {
        ...
        BotUtil.makeLog("warn", `HTTP/2不可用，回退到HTTP/1.1: ${err.message}`, '代理');
      }
    }
  ...
  _createProxyMiddleware(domainConfig) {
    const proxyOptions = {
      ...
      onProxyRes: (proxyRes, req, res) => {
        // 添加自定义响应头
        if (domainConfig.headers?.response && !res.headersSent) {
  ...
  _setupRequestLogging() {
    ...
    this.express.use((req, res, next) => {
      ...
      // 在响应发送前设置响应时间头（使用 res.once('close') 前的时机）
