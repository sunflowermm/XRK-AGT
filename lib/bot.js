...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  async run(options = {}) {
    ...
    // ========== 阶段0: 初始化配置 ==========
    BotUtil.makeLog('mark', '═══════════════════════════════════════════════════════════', '服务器');
    BotUtil.makeLog('mark', '葵子智能体启动中...', '服务器');
    BotUtil.makeLog('mark', '═══════════════════════════════════════════════════════════', '服务器');
    ...
    if (this.proxyEnabled) {
      ...
      BotUtil.makeLog('info', `服务端口：${this.httpPort} (HTTP), ${this.httpsPort} (HTTPS)`, '服务器');
    }
    
    if (this.proxyEnabled) {
      BotUtil.makeLog('info', '初始化反向代理应用...', '服务器');
      await this._initProxyApp();
    }
    
    // ========== 阶段1: 初始化基础服务（必须顺序执行） ==========
    BotUtil.makeLog('mark', '阶段1: 初始化基础服务', '服务器');
    
    BotUtil.makeLog('info', '加载包管理器...', '服务器');
    await Packageloader();
    
    BotUtil.makeLog('info', '生成API密钥...', '服务器');
    await this.generateApiKey();
    BotUtil.makeLog('info', '基础服务初始化完成', '服务器');
    
    // ========== 阶段2: 并行加载配置和模块（无依赖关系，可并行） ==========
    BotUtil.makeLog('mark', '阶段2: 加载配置和模块', '服务器');
    
    const ConfigLoader = (await import('./commonconfig/loader.js')).default;
    
    BotUtil.makeLog('info', '开始并行加载: 配置、工作流、插件、API', '服务器');
    ...
    BotUtil.makeLog('info', `启动HTTP服务器 (端口: ${this.httpPort})...`, '服务器');
    await this.serverLoad(false);
    BotUtil.makeLog('info', `HTTP服务器已启动 (端口: ${this.httpPort})`, '服务器');
    
    if (cfg.server.https.enabled) {
      BotUtil.makeLog('info', `启动HTTPS服务器 (端口: ${this.httpsPort})...`, '服务器');
      await this.httpsLoad();
      BotUtil.makeLog('info', `HTTPS服务器已启动 (端口: ${this.httpsPort})`, '服务器');
    }
    
    // 启动代理服务器
    if (this.proxyEnabled) {
      BotUtil.makeLog('info', '启动反向代理服务器...', '服务器');
      this.httpPort = originalHttpPort;
      this.httpsPort = originalHttpsPort;
      await this.startProxyServers();
      BotUtil.makeLog('info', '反向代理服务器已启动', '服务器');
    }
    
    // ========== 阶段6: 加载监听事件和适配器 ==========
    BotUtil.makeLog('mark', '阶段6: 加载监听事件和适配器', '服务器');
    await ListenerLoader.load();
    
    // ========== 阶段7: 启动文件监视 ==========
    BotUtil.makeLog('mark', '阶段7: 启动文件监视', '服务器');
    await ApiLoader.watch(true);
    
    // ========== 启动完成 ==========
    const loadTime = Date.now() - startTime;
    BotUtil.makeLog('mark', '═══════════════════════════════════════════════════════════', '服务器');
    BotUtil.makeLog('mark', `智能体启动完成 (耗时: ${loadTime}ms)`, '服务器');
