...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  wsConnect(req, socket, head) {
    ...
    // 解析WebSocket路径（去除查询参数）
    const urlPath = req.url.split("?")[0];
    const wsPath = urlPath.startsWith('/') ? urlPath.substring(1) : urlPath;
    
    // 记录WebSocket连接尝试
    BotUtil.makeLog('debug', `WebSocket连接请求: ${req.url} -> 路径: ${wsPath || '(根路径)'}`, '服务器');
    
    // 检查WebSocket路径是否存在
    if (!wsPath || !(wsPath in this.wsf) || !this.wsf[wsPath] || this.wsf[wsPath].length === 0) {
      const availablePaths = Object.keys(this.wsf).filter(k => this.wsf[k] && this.wsf[k].length > 0);
      BotUtil.makeLog('warn', `WebSocket路径未找到: ${wsPath}, 可用路径: [${availablePaths.join(', ') || '无'}]`, '服务器');
    ...
    this.wss.handleUpgrade(req, socket, head, conn => {
      ...
      // 使用规范化后的路径
      const normalizedWsPath = wsPath || urlPath.substring(1);
      if (this.wsf[normalizedWsPath] && this.wsf[normalizedWsPath].length > 0) {
        for (const handler of this.wsf[normalizedWsPath]) {
          handler(conn, req, socket, head);
        }
      } else {
        BotUtil.makeLog('error', `WebSocket处理器未找到: ${normalizedWsPath}`, '服务器');
      }
    });
  }
