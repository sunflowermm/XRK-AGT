...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  wsConnect(req, socket, head) {
    ...
    // 提取 WebSocket 路径，去除查询参数
    const urlPath = req.url.split("?")[0]; // 先去除查询参数
    const wsPath = urlPath.split("/")[1]; // 提取路径的第一段
    if (!wsPath || !(wsPath in this.wsf)) {
      BotUtil.makeLog("warn", `WebSocket路径未注册：${wsPath || req.url}，已注册的路径：${Object.keys(this.wsf).join(', ') || '无'}`, '服务器');
