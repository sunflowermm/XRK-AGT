...
export default class Bot extends EventEmitter {
  ...
  _initializeMiddlewareAndRoutes() {
    ...
    // 标记UI访问并下发轻量鉴权Cookie（仅用于同源前端调用API）
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  _authMiddleware(req, res, next) {
    ...
    // 来自本机前端UI（/xrk）下发的轻量Cookie，允许同源API无密钥访问
    try {
      ...
      if (hasUiCookie && sameOrigin) {
        BotUtil.makeLog("debug", `同源UI Cookie 认证通过：${req.method} ${req.originalUrl}`, '认证');
        return next();
      }
    } catch {}
  ...
  wsConnect(req, socket, head) {
    ...
    if (!isWhitelisted && !this._isLocalConnection(req.socket.remoteAddress)) {
      if (authConfig.apiKey?.enabled !== false) {
        ...
        if (!this._checkApiAuthorization(req)) {
          BotUtil.makeLog("error", `WebSocket认证失败：${req.url} (API Key: ${apiKey ? '已提供' : '未提供'})`, '服务器');
    ...
    // 提取WebSocket路径（去除查询参数和前导斜杠）
    const urlPath = req.url.split("?")[0]; // 去除查询参数
    const wsPath = urlPath.startsWith("/") ? urlPath.slice(1) : urlPath; // 去除前导斜杠
    if (!wsPath || !(wsPath in this.wsf)) {
      BotUtil.makeLog("warn", `WebSocket路径未找到: ${wsPath} (URL: ${req.url})`, '服务器');
      BotUtil.makeLog("debug", `已注册的WebSocket路径: ${Object.keys(this.wsf).join(', ')}`, '服务器');
  ...
  async run(options = {}) {
    ...
    console.log(chalk.cyan('╔════════════════════════════════════════════════════════════╗'));
    console.log(chalk.cyan('║') + chalk.yellow.bold('               葵子正在初始化http服务器...                  ') + chalk.cyan('║'));
    console.log(chalk.cyan('╚════════════════════════════════════════════════════════════╝'));
    ...
    // 先加载 ConfigManager，确保在 API 注册前可用
    const ConfigLoader = (await import('./commonconfig/loader.js')).default;
    await ConfigLoader.load();
    global.ConfigManager = ConfigLoader;
    
    // 验证 ConfigManager 是否正确加载
    const configCount = ConfigLoader.configs?.size || 0;
    if (configCount === 0) {
      BotUtil.makeLog('warn', 'ConfigManager 已加载但未找到任何配置', '服务器');
    } else {
      BotUtil.makeLog('info', `ConfigManager 已初始化并设置为全局变量，共加载 ${configCount} 个配置`, '服务器');
      // 列出所有已加载的配置
      const configNames = Array.from(ConfigLoader.configs.keys()).join(', ');
      BotUtil.makeLog('debug', `已加载的配置: ${configNames}`, '服务器');
    }
