...
export default class Bot extends EventEmitter {
  ...
  async _createHttpsProxyServer() {
    ...
    if (tlsConfig.http2 === true) {
      const http2 = await import('http2').catch(() => null);
      if (http2) {
      ...
      BotUtil.makeLog("warn", "HTTP/2不可用，回退到HTTP/1.1", '代理');
    }
        ...
        setHeaders: (res, filePath) => {
          try {
            ...
            if (err.code !== 'ERR_HTTP_HEADERS_SENT') {
              BotUtil.makeLog('warn', `设置静态文件响应头失败: ${err.message}`, '服务器');
            }
          }
        }
      };
      
      // 包装express.static以捕获错误并确保正确传递
      const staticMiddleware = express.static(staticRoot, staticOptions);
...
Sitemap: ${this.getServerUrl()}/sitemap.xml`;
  ...
  _fileHandler(req, res) {
    ...
    if (!file) {
      ...
      if (!file) {
        if (!res.headersSent) {
          res.status(404).json({ error: '未找到', file: url });
        }
        return;
      }
    }
  ...
  wsConnect(req, socket, head) {
    ...
    const wsPath = path.split("/")[1];
    
    // 检查路径是否存在
    if (!wsPath || !(wsPath in this.wsf)) {
      BotUtil.makeLog("warn", `WebSocket路径不存在：${path}`, '服务器');
    ...
    this.wss.handleUpgrade(req, socket, head, conn => {
      ...
      conn.on("error", err => BotUtil.makeLog("error", `WebSocket错误：${err.message}`, '服务器'));
  ...
  async httpsLoad() {
    ...
    if (tlsConfig.http2 === true) {
      ...
      if (http2) {
        ...
        BotUtil.makeLog("warn", "HTTP/2不可用，回退到HTTP/1.1", '服务器');
  ...
  _setupFinalHandlers() {
    ...
    this.express.use((err, req, res, next) => {
      if (res.headersSent) {
        BotUtil.makeLog('error', `响应已发送，无法处理错误: ${err.message}`, '服务器', err);
        return next(err);
      }
  ...
  async getLocalIpAddress() {
    ...
    result.primary = await this._getIpByUdp().catch(() => null);
    if (result.primary) {
    ...
    if (cfg.server?.misc?.detectPublicIP !== false) {
      result.public = await this._getPublicIP().catch(() => null);
    }
