...
const createBaseSegment = () => ({
  ...
  audio: (file) => ({ type: 'audio', data: { file } }),
  record: (file) => ({ type: 'record', data: { file } }),
  file: (file) => ({ type: 'file', data: { file } }),
...
// 初始化segment对象（使用基础实现，oicq可选）
let segment = createBaseSegment()

// 尝试加载oicq segment（异步，不阻塞）
(async () => {
  try {
    // 尝试导入oicq segment（可选）
    const oicqModule = await import('oicq').catch(() => null)
    if (oicqModule?.segment) {
      segment = oicqModule.segment
      // 如果oicq可用，也尝试导入icqq的segment（如果存在）
      try {
        const icqqPath = `file://${process.cwd()}/plugins/ICQQ-Plugin/node_modules/icqq/lib/message/elements.js`
        const { segment: icqq_segment } = await import(icqqPath).catch(() => ({}))
        if (icqq_segment) {
          // 合并icqq的segment方法（如果oicq没有的话）
          for (const i in icqq_segment) {
            if (!segment[i]) {
              segment[i] = icqq_segment[i]
            }
          }
        }
      } catch (e) {
    ...
    if (typeof logger !== 'undefined') {
      logger.debug?.('oicq segment不可用，使用基础segment实现')
    }
  }
  // 更新global.segment
...
class PluginsLoader {
  ...
  async parseMessage(e) {
    for (const val of e.message) {
      ...
      switch (val.type) {
        ...
        case 'at':
          // 通用ID获取（支持不同适配器）
          const id = val.user_id || val.qq || val.id || val.data?.user_id || val.data?.qq
          const botId = e.bot_id || e.self_id || e.bot?.uin || e.bot?.tiny_id || e.bot?.bot_id
          if (id && botId && String(id) === String(botId)) {
            e.atBot = true
          } else if (id) {
            e.at = id
            e.atList.push(id)
  ...
  checkPermissions(e) {
    ...
    const masters = masterUsers.length > 0 ? masterUsers : masterQQ
    const masterList = Array.isArray(masters) ? masters : [masters]
    
    // 也支持适配器特定的master配置
    const adapterMaster = cfg.master?.[e.self_id] || cfg.master?.[e.bot_id] || []
    const adapterMasterList = Array.isArray(adapterMaster) ? adapterMaster : [adapterMaster]
    
    // 合并所有主人列表
    const allMasters = [...masterList, ...adapterMasterList]

    if (allMasters.some(id => String(e.user_id) === String(id))) {
      e.isMaster = true
    }
  ...
  async preCheck(e, hasBypassPlugin = false) {
    try {
      ...
      if (isStartCommand) {
        ...
        const adapterMasterList = Array.isArray(adapterMaster) ? adapterMaster : [adapterMaster]
        const allMasters = [...masterList, ...adapterMasterList]
        const isMaster = allMasters.some(id => String(e.user_id) === String(id))
  ...
  checkBlack(e) {
    ...
    // 用户黑名单（通用配置）
    const blackUsers = other.blackUsers || []
    // 向后兼容：支持旧的blackQQ配置
    const blackQQ = other.blackQQ || []
    const blacklist = blackUsers.length > 0 ? blackUsers : blackQQ
    
    if (Array.isArray(blacklist) && blacklist.length > 0) {
      if (check(e.user_id).some(id => blacklist.includes(id))) return false
      if (e.at && check(e.at).some(id => blacklist.includes(id))) return false
      // 支持@的用户ID数组
      if (Array.isArray(e.at)) {
        if (e.at.some(atId => check(atId).some(id => blacklist.includes(id)))) return false
      }
    }

    // 设备黑名单
    const blackDevice = other.blackDevice || []
    if (e.device_id && Array.isArray(blackDevice) && blackDevice.length > 0) {
      if (blackDevice.includes(String(e.device_id)) || blackDevice.includes(Number(e.device_id))) {
        return false
      }
    }

    // 用户白名单（通用配置）
    const whiteUsers = other.whiteUsers || []
    // 向后兼容：支持旧的whiteQQ配置
    const whiteQQ = other.whiteQQ || []
    const whitelist = whiteUsers.length > 0 ? whiteUsers : whiteQQ
    
    if (Array.isArray(whitelist) && whitelist.length > 0 &&
      !check(e.user_id).some(id => whitelist.includes(id))) {
      return false
    }
