    ...
    for (const [key, config] of this.configs.entries()) {
      try {
        if (config && typeof config.getStructure === 'function') {
          ...
          if (!structure.displayName && config.displayName) {
            structure.displayName = config.displayName;
          }
          list.push(structure);
        } else {
          // 如果配置没有 getStructure 方法，提供基本信息
          BotUtil.makeLog('warn', `配置 ${key} 没有 getStructure 方法，使用基本信息`, 'ConfigLoader');
          list.push({
            name: key,
            displayName: config?.displayName || key,
            description: config?.description || '',
            filePath: config?.filePath || '',
            fileType: config?.fileType || 'yaml'
          });
        }
      } catch (error) {
        BotUtil.makeLog('error', `获取配置 ${key} 的结构失败`, 'ConfigLoader', error);
        // 即使出错也添加基本信息
        list.push({
          name: key,
          displayName: key,
