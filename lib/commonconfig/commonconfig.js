  ...
  _resolveFilePath() {
    if (this._getFilePath) {
      ...
      try {
        ...
        // 方式1: 从 global.cfg 获取
        if (typeof global !== 'undefined' && global.cfg) {
          cfg = global.cfg;
        }
        
        // 方式2: 如果 global.cfg 不存在，尝试从环境变量获取端口
        if (!cfg) {
          const port = process.env.SERVER_PORT || process.env.PORT || 8086;
          cfg = { _port: parseInt(port) };
        }
        ...
        BotUtil.makeLog('warn', `无法解析动态路径 [${this.name}]: ${error.message}`, 'ConfigBase');
        // 降级处理：使用默认端口
        try {
          const defaultPort = 8086;
          const dynamicPath = this._getFilePath({ _port: defaultPort });
          return path.join(process.cwd(), dynamicPath);
        } catch (e) {
          // 如果还是失败，返回默认路径
        }
