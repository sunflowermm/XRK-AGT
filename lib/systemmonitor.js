...
class SystemMonitor extends EventEmitter {
    ...
    async start(config) {
        ...
        // 初始化内存泄漏检测配置
        const leakConfig = config?.memory?.leakDetection || {};
        this.leakDetection.enabled = leakConfig.enabled !== false;
        this.leakDetection.threshold = leakConfig.threshold || 0.1;
        this.leakDetection.checkInterval = leakConfig.checkInterval || 300000;
    ...
    detectMemoryLeak() {
        ...
        if (avgGrowth > this.leakDetection.threshold) {
            logger.warn(`⚠️ 检测到潜在内存泄漏: 内存增长 ${(avgGrowth * 100).toFixed(2)}%`);
            this.emit('leak', {
                growth: avgGrowth,
            ...
            return { growth: avgGrowth, current: heapUsed, baseline: this.leakDetection.baseline };
        }
