    ...
    for (const api of this.priority) {
      try {
        ...
        if (api.routes && Array.isArray(api.routes)) {
          BotUtil.makeLog('debug', `准备注册API: ${apiName}, 路由数量: ${api.routes.length}`, 'ApiLoader');
          // 记录每个路由的详细信息
          api.routes.forEach((route, index) => {
            if (route && route.path && route.method) {
              BotUtil.makeLog('debug', `  - 路由 ${index + 1}: ${route.method.toUpperCase()} ${route.path}`, 'ApiLoader');
            } else {
              BotUtil.makeLog('warn', `  - 路由 ${index + 1} 配置不完整`, 'ApiLoader');
            }
          });
        }
        ...
        if (typeof api.init === 'function') {
          ...
          BotUtil.makeLog('warn', `API ${apiName} 没有 init 方法，跳过初始化`, 'ApiLoader');
        }
        
        // 记录路由和WebSocket注册信息
        const routeCount = api.routes ? api.routes.length : 0;
        const wsCount = api.wsHandlers ? Object.keys(api.wsHandlers).length : 0;
        const wsPaths = api.wsHandlers ? Object.keys(api.wsHandlers).join(', ') : '无';
        
        BotUtil.makeLog('info', `✓ 注册API: ${apiName} (优先级: ${api.priority || 100}, 路由: ${routeCount}, WebSocket: ${wsCount} [${wsPaths}])`, 'ApiLoader');
      } catch (error) {
        const apiName = api.name || api.key || 'undefined';
        BotUtil.makeLog('error', `注册API失败: ${apiName}`, 'ApiLoader', error);
        BotUtil.makeLog('error', `错误堆栈: ${error.stack}`, 'ApiLoader');
      }
    }
