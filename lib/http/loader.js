...
class ApiLoader {
  ...
  async load() {
    ...
    try {
      ...
      BotUtil.makeLog('info', `加载完成，共${this.apis.size}个API，耗时${loadTime}ms`, 'ApiLoader');
      
      return this.apis;
    } catch (error) {
      BotUtil.makeLog('error', '加载失败', 'ApiLoader', error);
      throw error;
    }
  }
  ...
  async getApiFiles(dir) {
    ...
    try {
      ...
      BotUtil.makeLog('error', `读取目录失败: ${dir}`, 'ApiLoader', error);
    }
    ...
    try {
      ...
      if (typeof module.default === 'function') {
        ...
        try {
          ...
          // 如果不能实例化，可能是普通函数，跳过
          BotUtil.makeLog('warn', `无法实例化API模块: ${key}`, 'ApiLoader');
          return false;
        }
      ...
      BotUtil.makeLog('debug', `加载API: ${apiInstance.name || key}`, 'ApiLoader');
      return true;
    } catch (error) {
  ...
  async register(app, bot) {
    ...
    const registeredWebSockets = new Set();
    
    // 按优先级顺序初始化API
    for (const api of this.priority) {
      try {
        if (!api || api.enable === false) {
          BotUtil.makeLog('debug', `跳过已禁用的API: ${api?.name || api?.key || 'unknown'}`, 'ApiLoader');
          continue;
        }
        ...
        if (typeof api.init === 'function') {
          ...
          // 如果没有自定义init方法，使用HttpApi的默认init
          const HttpApi = (await import('./http.js')).default;
          const apiInstance = new HttpApi(api);
          await apiInstance.init(app, bot);
        }
        ...
        if (api.ws && typeof api.ws === 'object') {
          for (const path of Object.keys(api.ws)) {
            registeredWebSockets.add(path);
            wsCount++;
          }
        }
        
        successCount++;
        BotUtil.makeLog('info', `✓ 注册API: ${apiName} (优先级: ${api.priority || 100}, 路由: ${api.routes?.length || 0}, WebSocket: ${Object.keys(api.ws || {}).length})`, 'ApiLoader');
      } catch (error) {
        failCount++;
        const apiName = api?.name || api?.key || 'undefined';
        ...
        // 输出错误堆栈（仅在调试模式下）
        if (cfg?.debug && error.stack) {
          BotUtil.makeLog('debug', error.stack, 'ApiLoader');
        }
      }
    }
    
    // 输出注册统计
    BotUtil.makeLog('info', `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`, 'ApiLoader');
    BotUtil.makeLog('info', `API注册统计:`, 'ApiLoader');
    BotUtil.makeLog('info', `  成功: ${successCount} 个`, 'ApiLoader');
    if (failCount > 0) {
      BotUtil.makeLog('warn', `  失败: ${failCount} 个`, 'ApiLoader');
    }
    BotUtil.makeLog('info', `  路由: ${routeCount} 个`, 'ApiLoader');
    BotUtil.makeLog('info', `  WebSocket: ${wsCount} 个`, 'ApiLoader');
    
    // 输出已注册的路由列表（调试模式）
    if (cfg?.debug && registeredRoutes.size > 0) {
      BotUtil.makeLog('debug', `已注册的路由:`, 'ApiLoader');
      Array.from(registeredRoutes).sort().forEach(route => {
        BotUtil.makeLog('debug', `  ${route}`, 'ApiLoader');
      });
    }
    
    // 输出已注册的WebSocket列表（调试模式）
    if (cfg?.debug && registeredWebSockets.size > 0) {
      BotUtil.makeLog('debug', `已注册的WebSocket:`, 'ApiLoader');
      Array.from(registeredWebSockets).sort().forEach(ws => {
        BotUtil.makeLog('debug', `  /${ws}`, 'ApiLoader');
      });
    }
