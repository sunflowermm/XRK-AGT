...
class ApiLoader {
  ...
  async register(app, bot) {
    ...
    for (const api of this.priority) {
      try {
        ...
        if (typeof api.init === 'function') {
          const result = await api.init(app, bot);
          if (result !== false) {
            successCount++;
            BotUtil.makeLog('info', `✓ 注册API: ${apiName} (优先级: ${api.priority || 100})`, 'ApiLoader');
          } else {
            failCount++;
    ...
    // 404处理（必须在所有路由之后）
    app.use('/api/*', (req, res) => {
      BotUtil.makeLog('warn', `API路由未找到: ${req.method} ${req.originalUrl}`, 'ApiLoader');
      res.status(404).json({
        success: false,
        message: 'API endpoint not found',
        path: req.originalUrl,
        availableApis: Array.from(this.apis.keys()),
        timestamp: Date.now()
      });
    });
    
    BotUtil.makeLog('info', `API注册完成: 成功 ${successCount} 个, 失败 ${failCount} 个`, 'ApiLoader');
  }
