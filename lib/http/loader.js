...
class ApiLoader {
  ...
  async register(app, bot) {
    ...
    for (const api of this.priority) {
      try {
        if (!api || api.enable === false) {
          BotUtil.makeLog('debug', `跳过禁用的API: ${api?.name || api?.key || 'unknown'}`, 'ApiLoader');
          continue;
        }
        
        // 确保API有name属性
        const apiName = api.name || api.key || 'undefined';
        const apiPriority = api.priority || 100;
        const routeCount = api.routes ? api.routes.length : 0;
        
        BotUtil.makeLog('info', `正在注册API: ${apiName} (优先级: ${apiPriority}, 路由数: ${routeCount})`, 'ApiLoader');
        
        // 初始化API
        if (typeof api.init === 'function') {
          const initResult = await api.init(app, bot);
          if (initResult === false) {
            throw new Error('API初始化返回false');
          }
        }
        ...
        if (api.ws && typeof api.ws === 'object') {
          const wsPaths = Object.keys(api.ws);
          if (wsPaths.length > 0) {
            BotUtil.makeLog('info', `  └─ WebSocket路径: ${wsPaths.join(', ')}`, 'ApiLoader');
          }
        }
        
        successCount++;
        BotUtil.makeLog('info', `✓ API注册成功: ${apiName}`, 'ApiLoader');
      } catch (error) {
        const apiName = api?.name || api?.key || 'undefined';
        failCount++;
        BotUtil.makeLog('error', `✗ 注册API失败: ${apiName}`, 'ApiLoader', error);
      }
    }
    
    // 404处理 - 必须在所有API路由注册之后
    app.use('/api/*', (req, res) => {
      BotUtil.makeLog('warn', `API端点未找到: ${req.method} ${req.originalUrl}`, 'ApiLoader');
      res.status(404).json({
        success: false,
    ...
    BotUtil.makeLog('info', `API注册完成: 成功 ${successCount}, 失败 ${failCount}, 总路由 ${registeredRoutes.size}`, 'ApiLoader');
    BotUtil.makeLog('mark', '━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'ApiLoader');
  }
