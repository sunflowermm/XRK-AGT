...
export default class HttpApi {
  ...
  registerRoutes(app, bot) {
    if (!Array.isArray(this.routes)) {
      BotUtil.makeLog('debug', `[HttpApi] ${this.name} 没有路由配置`, 'HttpApi');
      return;
    }
    ...
    if (registeredCount > 0) {
      BotUtil.makeLog('info', `[HttpApi] ${this.name} 共注册 ${registeredCount} 个路由`, 'HttpApi');
    }
  }
  ...
  registerWebSocketHandlers(bot) {
    if (!this.wsHandlers || typeof this.wsHandlers !== 'object') {
      BotUtil.makeLog('debug', `[HttpApi] ${this.name} 没有WebSocket处理器`, 'HttpApi');
      return;
    }
    ...
    for (const [path, handlers] of Object.entries(this.wsHandlers)) {
      ...
      for (const handler of handlerArray) {
        if (typeof handler === 'function') {
          ...
          BotUtil.makeLog('info', `[HttpApi] ${this.name} 注册WebSocket处理器: /${path}`, 'HttpApi');
        } else {
          BotUtil.makeLog('warn', `[HttpApi] ${this.name} WebSocket处理器无效 (路径: ${path}): 不是函数`, 'HttpApi');
        }
      }
    }
  }
