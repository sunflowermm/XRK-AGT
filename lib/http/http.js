...
export default class HttpApi {
  ...
  async init(app, bot) {
    try {
      BotUtil.makeLog('debug', `[HttpApi] ${this.name} 开始初始化`, 'HttpApi');
      
      // 注册中间件
      if (this.middleware && this.middleware.length > 0) {
        BotUtil.makeLog('debug', `[HttpApi] ${this.name} 注册 ${this.middleware.length} 个中间件`, 'HttpApi');
      ...
      if (typeof this.initHook === 'function') {
        BotUtil.makeLog('debug', `[HttpApi] ${this.name} 执行自定义初始化`, 'HttpApi');
        await this.initHook(app, bot);
      }
      
      BotUtil.makeLog('info', `[HttpApi] ${this.name} 初始化完成`, 'HttpApi');
      return true;
    } catch (error) {
      BotUtil.makeLog('error', `[HttpApi] ${this.name} 初始化失败:`, 'HttpApi', error);
      BotUtil.makeLog('error', `[HttpApi] 错误堆栈: ${error.stack}`, 'HttpApi');
      return false;
    }
  }
  ...
  registerRoutes(app, bot) {
    if (!Array.isArray(this.routes)) {
      BotUtil.makeLog('warn', `[HttpApi] ${this.name} routes 不是数组，跳过路由注册`, 'HttpApi');
      return;
    }
    ...
    BotUtil.makeLog('debug', `[HttpApi] ${this.name} 开始注册 ${this.routes.length} 个路由`, 'HttpApi');
    ...
    for (const route of this.routes) {
      try {
        ...
        if (!method || !path || !handler) {
          BotUtil.makeLog('warn', `[HttpApi] ${this.name} 路由配置不完整:`, 'HttpApi', route);
          continue;
        }
        ...
        BotUtil.makeLog('error', `[HttpApi] ${this.name} 注册路由失败:`, 'HttpApi', error);
        BotUtil.makeLog('error', `[HttpApi] 路由详情: ${JSON.stringify(route)}`, 'HttpApi');
      }
    }
    
    BotUtil.makeLog('info', `[HttpApi] ${this.name} 路由注册完成，共注册 ${this.routes.length} 个路由`, 'HttpApi');
  }
  ...
  wrapHandler(handler, bot) {
    return async (req, res, next) => {
      try {
        ...
        BotUtil.makeLog('error', `[HttpApi] ${this.name} 处理请求失败:`, 'HttpApi', error);
        ...
        if (typeof handler === 'function') {
          bot.wsf[path].push((conn, req, socket, head) => {
            try {
              ...
              BotUtil.makeLog('error', `[HttpApi] ${this.name} WebSocket处理失败:`, 'HttpApi', error);
            }
          });
          
          BotUtil.makeLog('debug', `[HttpApi] ${this.name} 注册WebSocket处理器: ${path}`, 'HttpApi');
        }
      }
    }
  }
