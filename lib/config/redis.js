...
function handleFinalConnectionFailure(error, port) {
  ...
  // 开发/无Redis环境降级：提供内存版Redis替代，避免进程直接退出导致循环重启
  logger.warn('未检测到可用的Redis，将启用内存Redis降级模式（仅当前进程有效）')
  const shim = createInMemoryRedisShim()
  globalClient = shim
  global.redis = shim
  return
}
...
/**
 * 内存版Redis简易实现（最小可用接口）
 * 仅用于开发或无Redis环境的降级，不可用于生产
 */
function createInMemoryRedisShim() {
  const store = new Map()
  const expirations = new Map()
  const now = () => Date.now()
  
  const setExpireTimer = (key, seconds) => {
