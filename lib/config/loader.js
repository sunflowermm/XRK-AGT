...
class ProcessManager {
  ...
  async updateTitle() {
    // 通用进程标题设置（不特定于QQ）
    const selectedMode = global.selectedMode || process.argv[2] || 'default';
    const selectedId = global.selectedId || process.argv[3] || '';
    
    if (selectedMode === "server") {
      process.title = selectedId 
        ? `${CONFIG.PROCESS_TITLE}@Server:${selectedId}` 
        : `${CONFIG.PROCESS_TITLE}@Server`;
    } else {
      process.title = selectedId 
        ? `${CONFIG.PROCESS_TITLE}@${selectedId}` 
  ...
  setupErrorHandlers() {
    ...
    process.on("exit", async (code) => {
      await this.cleanup();
      logger.mark(chalk.magenta(`XRK-AGT (葵子) 已停止，退出码: ${code}`));
    });
  }
...
class InitManager {
  ...
  async init() {
    await setLog();
    logger.mark(chalk.cyan("XRK-AGT (葵子) 初始化中..."));
    ...
    logger.mark(chalk.green(`XRK-AGT (葵子) 初始化完成`));
