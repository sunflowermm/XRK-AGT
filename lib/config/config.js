...
class Cfg {
  ...
  /**
   * 获取主人映射对象，用于向后兼容
   * 返回 {bot_id: [masterUsers数组]} 结构
   * @deprecated 建议直接使用 masterUsers
   */
  get master() {
    const masters = {};
    const masterList = this.masterUsers;

    if (typeof Bot !== 'undefined' && Bot.uin && Array.isArray(Bot.uin)) {
      for (const botId of Bot.uin) {
        masters[botId] = masterList;
      }
    } else {
      const currentBotId = this.bot.account?.uin || this.bot.account?.id || 'current_bot';
      masters[currentBotId] = masterList;
    }
