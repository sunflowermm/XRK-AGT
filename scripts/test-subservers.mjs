 ...
 *
 * 用法：
 *   node scripts/test-subservers.mjs
 *   node scripts/test-subservers.mjs --runtime goserver,jserver
 *   node scripts/test-subservers.mjs --host 127.0.0.1 --timeout 8000
 */
...
const PROBES = {
  ...
  rustserver: [
    ...
    {
      method: 'POST',
      path: '/api/regex-tools/match',
      body: { text: 'abc123', pattern: '\\d+' },
      expectOk: true
    }
  ]
};
...
async function main() {
  ...
  console.log(`子服冒烟测试 host=${host} timeout=${timeoutMs}ms\n`);
  ...
  for (const id of ids) {
    ...
    console.log(`[${mark}] ${id} (${report.label}) @ ${report.baseUrl}`);
    for (const r of report.results) {
      if (r.ok) console.log(`       ✓ ${r.label} ${r.status} ${r.ms}ms`);
      else console.log(`       ✗ ${r.label} — ${r.error}`);
    }
    if (!report.ok) console.log(`       → ${report.error}`);
    console.log('');
  }

  const passed = reports.filter((r) => r.ok).length;
  const total = reports.length;
  console.log(`合计: ${passed}/${total} runtime 通过`);

  if (passed < total) {
    console.log('\n提示: 见 subserver/SETUP.md');
    process.exitCode = 1;
  }
}
