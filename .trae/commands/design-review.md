...
- Prioritize review targets in `core/*/www/<app-name>/`
- If review touches API/UI collaboration, include related handlers in `core/*/http/*.js`
- Do not recommend moving business UI code into `src/infrastructure/`
- Recommendations should be additive and migration-friendly; avoid destructive rewrites by default
- If review identifies config mismatch, call out exact paths in:
...
2. Load project context if available:
   ...
   - Detect XRK core-specific front-end directories under `core/*/www/`
...
## XRK-AGT Report Requirements

Each review report should additionally include:

1. XRK module ownership (which `core/<core-name>/` owns the target UI)
2. Extension path recommendation (where to add new components without breaking existing loaders)
3. Config/schema sync risks if any UI behavior depends on runtime config
4. A "safe rollout" checklist: small-scope patch order and regression touchpoints
