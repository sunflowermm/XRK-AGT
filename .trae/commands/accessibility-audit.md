...
- Prioritize audits for UI under `core/*/www/<app-name>/`
- If accessibility behavior is controlled by config, verify consistency among:
  - `config/default_config/`
  - `core/system-Core/commonconfig/*.js`
  - runtime consumer code
- Keep remediation additive and safe: patch semantics/ARIA/focus/labels before large refactors
- Do not suggest moving business UI logic into `src/infrastructure/`
- For HTTP-coupled UX issues, include related `core/*/http/*.js` endpoints in scope
...
2. Load project context:
   ...
   - Detect XRK core ownership for the target UI (`core/<core-name>/www/...`)
...
## XRK-AGT Audit Report Addendum

Each audit report should additionally contain:

1. XRK ownership mapping (core + app path)
2. Risk note for config/schema mismatch if any
3. Safe rollout order (critical -> serious -> moderate)
4. Verification matrix:
   - keyboard-only
   - screen reader quick pass
   - contrast validation
   - no regression in existing XRK flows
