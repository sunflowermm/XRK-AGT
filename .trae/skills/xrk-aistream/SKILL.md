---
name: xrk-aistream
description: 当你需要开发/调试 AIStream 工作流、RAG 上下文增强、MCP 工具注册与作用域控制时使用。
---

## 文档与代码

`docs/aistream.md`、`src/infrastructure/aistream/aistream.js`、`loader.js`
...
子服务端为**可选** Python 扩展；AIStream 核心链路不依赖子服务。业务 Core 可通过 `Bot.callSubserver` 调用子服务 `apis/` 接口。
