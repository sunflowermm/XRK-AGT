...
- 浏览器截图共用 `BrowserRendererBase`：唯一 `slotId` 占槽（禁止用模板 name 入队）、排队超时、`priority`/`userTriggered` 用户优先槽；健康检查/重启看 `activeSlotCount()`。僵死实例靠 `ensureBrowserHealthy` + `safeCloseBrowser`（close 超时则 disconnect/SIGKILL）；截图 timeout/disconnected 走 `handleFatalScreenshotError` → `restart(true)`，勿等整进程重启。
