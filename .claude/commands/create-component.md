...
## XRK-AGT Alignment (Required)

- Prefer creating product components inside `core/*/www/<app-name>/` related component directories
- Do not place business components under `src/infrastructure/`
- Keep generation additive: introduce new component/variant files before replacing existing ones
- If component depends on configurable behavior, ensure consistency across:
  - `config/default_config/`
  - `core/system-Core/commonconfig/*.js`
  - consumer logic
- When path choice is ambiguous, ask with XRK-specific options first (e.g., `core/system-Core/www/...`)
...
2. Detect project configuration:
   ...
   - Detect XRK app component roots under `core/*/www/*`
...
1. core/system-Core/www/{app}/components/{ComponentName}/
2. core/{core-name}/www/{app}/components/{ComponentName}/
3. src/components/{ComponentName}/
4. app/components/{ComponentName}/
5. components/{ComponentName}/
...
## XRK-AGT Completion Addendum

Completion output should also confirm:

1. Final owning XRK core path (exact directory)
2. Whether imports/aliases follow current core package constraints
3. Whether a follow-up `design-review` and `accessibility-audit` is required for newly added variants
