...
- Prefer token/application landing path under `core/*/www/<app-name>/` for business UI usage
- Keep design system assets additive; do not remove existing core token definitions unless explicitly requested
- If setup introduces runtime configuration options, synchronize:
  - `config/default_config/` defaults
  - `core/system-Core/commonconfig/*.js` schema
  - actual consumer code
- Avoid relocating business UI logic into `src/infrastructure/`
- When uncertain where to place files, ask explicitly with concrete path options
...
3. Load project context:
   ...
   - Detect XRK web app roots under `core/*/www/`
   - Prefer existing XRK front-end app folders over creating generic root-level UI folders
...
In completion summary, also include:

1. Which `core/*/www/*` application(s) are now consuming tokens
2. Whether new config/schema wiring was needed (`default_config` + `commonconfig`)
3. Backward-compatibility statement (what existing UI remains untouched)
4. Suggested next XRK command sequence:
