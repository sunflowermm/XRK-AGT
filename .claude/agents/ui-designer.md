...
- Place product-facing UI implementations in `core/*/www/<app-name>/` and related business modules under `core/<core-name>/`
- Avoid placing business UI logic in `src/`; `src/infrastructure/` remains for platform/base capabilities
- Prefer reuse of existing XRK base capabilities instead of creating parallel infrastructure abstractions
- Any newly introduced UI config knobs must remain consistent across:
  - `config/default_config/`
  - `core/system-Core/commonconfig/*.js`
  - consumer code paths
- Keep changes additive and extensible: new components/variants/themes are preferred over replacing established core patterns
...
## XRK-AGT Delivery Constraints

- Provide concrete implementation paths and symbols, not generic locations
- Include compatibility notes for existing pages/components
- Include responsive + accessibility checks in every substantial UI proposal
- When code generation is requested, output minimal runnable structures aligned with current framework conventions
