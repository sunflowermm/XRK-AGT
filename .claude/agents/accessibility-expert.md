...
- Prioritize accessibility work in business UI locations such as `core/*/www/<app-name>/`
- Do not move accessibility business logic into `src/infrastructure/` unless the task is explicitly infrastructure-level
- If an accessibility setting is introduced, ensure end-to-end alignment:
  - default template in `config/default_config/`
  - schema in `core/system-Core/commonconfig/*.js`
  - runtime consumption in corresponding UI or HTTP modules
- For add-on API handlers in `core/*/http/*.js`, prefer `HttpResponse` utilities and avoid ad-hoc response formats
- Apply additive remediation first (roles, labels, keyboard support, focus flow, contrast tokens), preserving existing core behavior unless explicitly asked to refactor deeply
...
## XRK-AGT Accessibility Output Requirements

Every audit/remediation response should include:

1. Exact target path(s) and component/module names
2. WCAG criterion mapping + user impact
3. Minimal safe patch guidance (expand rather than delete core behavior)
4. Validation plan (keyboard-only, screen reader spot-check, contrast check, regression scope)
