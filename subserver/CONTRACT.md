...
| 职责 | 位置 |
|------|------|
| **编辑**（控制台） | 主服 `ConfigLoader`：`core/*/commonconfig/` + `subserver/*/apis/*/core/commonconfig/` |
| **子服连接 host/port** | 主服 `aistream.yaml` → `cfg.subserver`（系统配置 → AIStream → 子服务端） |
...
主服扫描 `subserver/<runtime>/apis/<group>/core/{commonconfig,plugin,...}`。**配置全在主服 CommonConfig 编辑**；业务 yaml 在 `data/<group>/`；子服 host/port 在 `aistream.yaml` → `cfg.subserver`。见 [docs/subserver-commonconfig.md](../docs/subserver-commonconfig.md)。
