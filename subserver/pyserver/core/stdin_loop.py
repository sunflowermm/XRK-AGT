...
def _setup_readline(prompt: str) -> None:
    rl = _load_readline()
    if rl is None:
        return
    ...
    def _completer(text: str, state: int) -> Optional[str]:
        if state == 0:
            cache["words"] = [w for w in completion_words(CommandRegistry.groups) if w.startswith(text)]
        idx = state
...
def _stdin_reader_loop(prompt: str) -> None:
    _setup_readline(prompt)
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

    with _io_lock:
        print("\n[子服] 帮助 · 列表 · 更新 · 清屏 · 退出（Tab 补全）", flush=True)
