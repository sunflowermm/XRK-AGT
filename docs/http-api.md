...
- **成功**：`HttpResponse.success(res, data, message)`：
  ...
  - **前端**：勿默认 `json.data` 再读字段；有 `data` 用 `data`，否则读顶层业务字段，或解包去掉 `success`/`message` 的剩余对象。反例：psyche 曾 `return json.data` 后读 `webVersion` → `undefined.webVersion`。ESM 页优先 `import { unwrapSuccess } from '/shared/xrk-web-compat.js'`（权威 skill **`xrk-www-compat`**）。
