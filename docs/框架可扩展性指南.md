...
| `events/` | 事件监听器 | `EventListener.js` | 可选 | 监听事件，去重后分发到插件 |
| `http/` | HTTP API | `HttpApi.js` | 可选 | REST API 和 WebSocket |
...
9. **导入路径规范**：必须使用别名路径（`#` 前缀），不要使用相对路径。可用别名：`#infrastructure/*`、`#utils/*`、`#factory/*`、`#config/*`、`#data/*`、`#core/*`。
