...
flowchart TB
    ...
    C --> D[配置Embedding<br/>provider/enabled]
    D --> E[实现init方法<br/>注册函数]
    E --> F[实现buildSystemPrompt<br/>构建系统提示]
    F --> G[实现buildChatContext<br/>构建上下文]
...
框架支持多种Embedding提供商，也可以扩展新的提供商：

```javascript
// 内置提供商
embedding: {
  enabled: true,
  provider: 'lightweight'  // 轻量级BM25算法
}

// 扩展新提供商（需要修改aistream.js）
// 在tryInitProvider方法中添加新的case
```
...
- **插件**: `core/plugin/example/my-plugin.js`
- **工作流**: `core/stream/my-workflow.js`
- **Tasker**: `core/tasker/MyTasker.js`
- **事件监听器**: `core/events/my-listener.js`
