...
2. **基类抽象**：见 [base-classes.md](base-classes.md)（plugin / HttpApi / AIStream / ConfigBase / EventListenerBase）
...
| 扩展点 | 目录 | 基类 / 约定 | 专题 |
|--------|------|-------------|------|
| 插件 | `core/*/plugin/` | `plugin` | [plugin-base.md](plugin-base.md) |
| 工作流 | `core/*/stream/` | `AIStream` | [aistream.md](aistream.md) |
| Tasker | `core/*/tasker/` | 注册 `Bot.tasker` | [tasker-base-spec.md](tasker-base-spec.md) |
| 事件监听 | `core/*/events/` | `EventListenerBase` | [事件系统标准化文档.md](事件系统标准化文档.md) |
...
- 对于 **自带 `package.json` 的独立 core 包**：必须使用 **相对路径** 访问 `src/*` 等其他包内代码，避免出现 `ERR_PACKAGE_IMPORT_NOT_DEFINED`。示例：从 `core/my-core/http/my-api.js` 导入 `src/utils/botutil.js` 应使用 `../../../src/utils/botutil.js`。
...
在 **自带 `package.json` 的独立 core 包** 中，由于 Node 解析 `imports` 只在当前包生效，访问 `src/*` 等主包代码时必须使用相对路径（否则会报 `ERR_PACKAGE_IMPORT_NOT_DEFINED`）。示例：从 `core/my-core/http/my-api.js` 导入 `src/utils/botutil.js` 应使用 `../../../src/utils/botutil.js`。
...
```mermaid
flowchart TB
    A["创建 EventListenerBase 子类"] --> B["constructor<br/>super('adapterName')"]
    B --> C["实现 init()<br/>Bot.on 注册监听"]
    C --> D["handleEvent<br/>markProcessed 去重"]
    D --> E["markAdapter(e, flags)"]
...
flowchart TB
    ...
    C --> D["可选 init(app, bot)"]
    D --> E["放置到 core/*/http/<br/>ApiLoader 自动加载"]
...
export default class MyWorkflow extends AIStream {
  constructor() {
    super({
      ...
      embedding: {
        enabled: true
        // embedding 配置自动从 cfg 读取，无需手动指定
        // embedding 段由 StreamLoader.applyEmbeddingConfig 自动合并，无需在 init() 中处理
      }
    });
  }
  ...
  async init() {
    await super.init();
    // embedding 配置由 StreamLoader 加载时合并，无需手动初始化向量服务
...
```javascript
// aistream.yaml → embedding（由 StreamLoader.applyEmbeddingConfig 合并到各工作流）
embedding: {
...
await mainStream.process(e, question, {
  ...
  enableMemory: true,         // 整合工具工作流（memory）
  enableDatabase: true,       // 整合工具工作流（database）
  enableTools: true          // 整合工具工作流（tools）
});
...
// 
// 合并后的工作流：
// - 包含所有工作流的MCP工具（工具工作流的工具自动整合）
// - 保留主工作流的人设和上下文（buildSystemPrompt、buildChatContext）
// - 使用合并后的名称作为记忆键（如 chat-desktop）
```

**工作流整合机制**：
- `mergeStreams`：合并主工作流（`device`、`chat`、`desktop`），合并MCP工具
- `enableMemory: true`：自动整合 `memory` 工具工作流
- `enableDatabase: true`：自动整合 `database` 工具工作流
- `enableTools: true`：自动整合 `tools` 工具工作流
- 工具工作流通过标志启用，自动整合MCP工具到主工作流
...
- 基类：`EventListenerBase`（`src/infrastructure/listener/base.js`），**不是** `listener/listener.js` 中的旧 `EventListener` 类（未被加载器使用）。
- `constructor(adapterName)` 只接收适配器名字符串；在 `init()` 里用 `Bot.on` / `this.bot.on` 注册事件。
- 去重用 `markProcessed(e)`；标记 Tasker 用 `markAdapter(e, { ...flags })`（第二参为对象）。
- 分发用 `this.plugins.deal(e)`（基类已注入 `PluginsLoader`）。
...
export default class MyTaskerEvent extends EventListenerBase {
  ...
  async handleEvent(e) {
    ...
    this.markAdapter(e, { isMyTasker: true });
...
### 子服业务插件（CommonConfig 代理）

业务子服插件（如 `subserver/pyserver/apis/jmcomic/`）**不在主服** `core/*/commonconfig/` 维护 schema：

| 层级 | 职责 |
|------|------|
| 子服 | `default_config.yaml`、`config_schema.yaml`、`default.plugin_config` → HTTP `/api/{group}/config/*` |
| 主服 | `ConfigLoader.registerFromSubserver()` → `SubserverConfigProxy`；控制台与本地 Core 共用 UI |
| 连接 | `aistream.yaml` → `subserver` → **`cfg.subserver`** → `Bot.callSubserver` |

详见 [subserver-commonconfig.md](subserver-commonconfig.md)、[subserver-plugin-development.md](subserver-plugin-development.md)、[subserver/CONTRACT.md](../subserver/CONTRACT.md)。
...
export default class CompletePlugin extends plugin {
  ...
  async scheduledTask() {
    Bot.makeLog('info', '定时任务执行', 'CompletePlugin');
  }
...
export default class CompleteWorkflow extends AIStream {
  constructor() {
    super({
      ...
      embedding: {
        enabled: true
        // enabled / maxContexts 等自动从 cfg.aistream.embedding 合并
        // embedding 段由 StreamLoader.applyEmbeddingConfig 自动合并，无需在 init() 中处理
      }
    });
  }
...
- **[AIStream 文档](aistream.md)** · **[插件基类文档](plugin-base.md)**
