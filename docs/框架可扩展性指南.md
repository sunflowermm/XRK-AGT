...
flowchart TB
    ...
    Extension -->|继承| Infrastructure
    Infrastructure -->|提供能力| Extension
...
> 权威清单：[node-26-runtime.md](node-26-runtime.md) · Cursor skill：`xrk-node-runtime` · 规则：`xrk-core-code.mdc`

扩展 Core 代码时**必须使用**项目已统一的 Node 26 API，**禁止** AI 或开发者重新引入旧依赖/旧模式：

| 场景 | 使用 | 禁止 |
|------|------|------|
| HTTP 出站 | 全局 `fetch` + `AbortSignal.timeout` | `node-fetch` |
| LLM 代理 | `buildFetchOptionsWithProxy` | `https-proxy-agent` |
| Shell | `#utils/exec-async.js` | `util.promisify(exec)`、`child_process/promises` |
| 判错 | `Error.isError`、`normalizeError` | `instanceof Error` |
| 二进制 | `toBase64()` / `fromBase64()` | `toString('base64'|'hex')` |

**不要**在 Core 中修改 `src/infrastructure/`；若缺能力，应提 issue 或在基础设施层统一扩展后再在 Core 复用。
...
- **[Node 26 运行时约定](node-26-runtime.md)** - 版本要求、API 清单、禁止旧写法
