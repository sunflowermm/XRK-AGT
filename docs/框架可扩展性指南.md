...
2. **基类抽象**：见 [base-classes.md](base-classes.md)（plugin / HttpApi / AiWorkflow / ConfigBase / EventListenerBase）
...
| 工作流 | `core/*/stream/` | `AiWorkflow` | [aistream.md](aistream.md) |
...
| `stream/` | AI 工作流 | `AiWorkflow.js` | 可选 | AI 工作流实现 |
...
// 获取其他 core 的工作流
const chatStream = AiStreamLoader.getStream('chat'); // 来自 system-Core
...
5. **跨 Core 资源共享**：不同 core 的资源可以相互使用。例如：
   ...
   - 通过加载器（如 `AiStreamLoader.getStream()`）获取其他 core 的资源
...
export default class MyWorkflow extends AiWorkflow {
  constructor() {
    super({
      ...
      embedding: {
        enabled: true
        // embedding 配置自动从 runtimeConfig 读取，无需手动指定
        // embedding 段由 AiStreamLoader.applyEmbeddingConfig 自动合并，无需在 init() 中处理
      }
    });
  }
  ...
  async init() {
    await super.init();
    // embedding 配置由 AiStreamLoader 加载时合并，无需手动初始化向量服务
...
Embedding 配置自动从 `runtimeConfig.aistream.embedding` 读取，工作流只需设置 `enabled`：

```javascript
// aistream.yaml → embedding（由 AiStreamLoader.applyEmbeddingConfig 合并到各工作流）
embedding: {
...
```javascript
// 主工作流
const mainStream = AiStreamLoader.getStream('chat');
...
// 推荐唯一组合路径：mergeStreams → AiStreamLoader.mergeStreams（不 mutate 主单例）
...
- `constructor(adapterName)` 只接收适配器名字符串；在 `init()` 里用 `AgentRuntime.on` / `this.bot.on` 注册事件。
...
// 测试工作流
const stream = AiStreamLoader.getStream('my-workflow');
...
export default class CompleteWorkflow extends AiWorkflow {
  constructor() {
    super({
      ...
      embedding: {
        enabled: true
        // enabled / maxContexts 等自动从 runtimeConfig.aistream.embedding 合并
        // embedding 段由 AiStreamLoader.applyEmbeddingConfig 自动合并，无需在 init() 中处理
      }
    });
  }
...
| 工作流 | `ai-workflow.js` | `core/*/stream/` | ✅ | ✅ | ✅ |
| Tasker | - | `core/*/tasker/` | ✅ | - | ✅ |
...
- **[AiWorkflow 文档](aistream.md)** · **[插件基类文档](plugin-base.md)**
