...
- **`www/` 不是唯一合法客户端目录**：
  ...
  - 其他类型的客户端（如小程序、独立 Web 项目、桌面客户端等）可以放在 Core 自己约定的子目录下，只要不与框架保留目录（`plugin/`、`workflow/`、`events/`、`tasker/`、`commonconfig/` 等）冲突即可
- 对于这类**独立子项目**，推荐做法：
...
```mermaid
flowchart TB
    A["创建 ListenerBase 子类"] --> B["constructor<br/>super('adapterName')"]
...
- 基类：`ListenerBase`（`src/infrastructure/listener/base.js`），**不是** `listener/listener.js` 中的旧 `EventListener` 类（未被加载器使用）。
