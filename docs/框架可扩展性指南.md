...
// 推荐唯一组合路径：mergeStreams → StreamLoader.mergeStreams（不 mutate 主单例）
// enableMemory/Database/Tools 仅无 mergeStreams 时的兼容别名
// web/browser 等声明 frameworkToolSurface 的流自动进 chat 工具白名单
```

**工作流整合机制**：
- `mergeStreams`：副流名称列表（`memory`、`database`、`tools`、`desktop` 等）
- `enableMemory` / `enableDatabase` / `enableTools`：兼容别名 → 映射为副流后仍走 `mergeStreams`
