...
当配置需要按类型拆分为多个文件时（如渲染器配置包含 puppeteer 和 playwright），可以使用 `multiFile` 特性：
...
// 返回：{ 'puppeteer.headless': 'new', 'playwright.browserType': 'chromium', ... }
```

**多文件配置的优势**：
- ✅ 无需创建特殊类，统一使用 `ConfigBase`
- ✅ 可扩展：添加新的多文件配置只需在 metadata 中定义 `multiFile`
- ✅ 统一接口：与单文件配置使用相同的 API
- ✅ 前端兼容：扁平化后的数据格式与单文件配置一致
...
// 支持函数式路径
filePath: (cfg) => {
  return `data/server_bots/${cfg.port || cfg._port}/myconfig.yaml`;
}
```
