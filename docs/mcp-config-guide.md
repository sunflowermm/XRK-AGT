...
XRK-AGT 提供标准化的 **Model Context Protocol (MCP)** 服务，允许外部 AI 平台（如 Cursor、Claude Desktop、小智 AI）通过 HTTP 连接并调用系统工具。架构与工具列表见 [MCP 完整指南](mcp-guide.md)。
...
MCP 配置定义外部平台如何连接 XRK-AGT、选用哪些工作流工具及 URL/传输参数。协议与注册机制见 [MCP 完整指南](mcp-guide.md)。
...
| 工作流 | 说明 | 主要工具 |
|--------|------|----------|
| `tools` | 文件操作工具 | `read`, `grep`, `search_replace`, `write`, `delete_file`, `list_files`, `run` |
