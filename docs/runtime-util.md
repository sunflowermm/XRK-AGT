...
> **文件位置**：`src/utils/runtime-util.js`  
> **说明**：`RuntimeUtil` 是 XRK-AGT 的核心工具类，提供 **日志封装、缓存管理、文件系统操作、网络请求、批处理与重试、时间和大小格式化、消息辅助等** 能力，被 `AgentRuntime`、Tasker、插件和 API 广泛使用。
...
| 项目 | 内容 |
|------|------|
| 所在文件 | `src/utils/runtime-util.js` |
| 类型 | 全静态工具类（`export default class RuntimeUtil`） |
| 典型使用方式 | `import RuntimeUtil from '#utils/runtime-util.js'` |
| 主要服务对象 | `AgentRuntime`、插件（`plugin`）、Tasker、`HttpApiLoader` 等 |

### 相关底层模块（Node 26）

| 模块 | 路径 | 说明 |
|------|------|------|
| `exec-async` | `src/utils/exec-async.js` | Promise 版 `exec`；Core 通过 `RuntimeUtil.exec` 或 `#utils/exec-async.js` 使用 |
...
| 属性 | 类型 | 说明 |
|------|------|------|
| `apiKey` | `string` | 由 `AgentRuntime.generateApiKey()` 填充的 API 密钥，全局共享 |
...
| `#memoryCache` | `Map<string,object>` | 简单 KV 缓存，用于 `RuntimeUtil.cache` |
...
classDiagram
    ...
    RuntimeUtil --> ExtendedMap : 创建
    RuntimeUtil --> MemoryCache : 使用
    
    style RuntimeUtil fill:#E3F2FD,stroke:#1976D2,stroke-width:2px
...
- `制作聊天记录(...)` → `RuntimeUtil.makeChatRecord`
...
// 获取命名Map（带TTL和LRU）
const cache = RuntimeUtil.getMap('my-cache', {
  maxSize: 1000,
  ttl: 3600000  // 1小时
});
...
// 轻量缓存
RuntimeUtil.cache('key', 'value', 60000);  // 缓存1分钟
const value = RuntimeUtil.cache('key');    // 获取缓存
```
...
// 写入文件（自动创建目录）
await RuntimeUtil.writeFile('data/output.txt', 'content');

// 检查文件是否存在
if (await RuntimeUtil.fileExists('data/file.txt')) {
  // 文件存在
}

// 文件转URL（用于发送图片等）
const url = await RuntimeUtil.fileToUrl(buffer, {
  name: 'image.png',
});
```
...
```javascript
const ret = await RuntimeUtil.exec('node -v', { quiet: true });
...
await RuntimeUtil.promiseEvent(emitter, 'ready', 'error', 5000);
```
...
```javascript
// 生成日志ID
const logId = RuntimeUtil.makeLogID('123456');
// 输出: [123456] (带颜色)

// 输出日志
RuntimeUtil.makeLog('info', '这是一条信息', '123456');
RuntimeUtil.makeLog('error', '错误信息', '123456', true);  // 包含堆栈
```
...
```javascript
// 时间差格式化
const diff = RuntimeUtil.getTimeDiff(startTime, endTime);
// 输出: "2天 3小时 15分钟 30秒"

// 文件大小格式化
const size = RuntimeUtil.formatFileSize(1024 * 1024 * 5);
// 输出: "5.00 MB"
```
...
- **`AgentRuntime`（`src/agent-runtime.js`）**
  - 使用 `RuntimeUtil.getMap` 管理本地 IP 缓存等。
  - 使用 `RuntimeUtil.makeLog` 统一输出 HTTP/WS/代理/认证等日志。
  - 借助 `RuntimeUtil.fileToUrl/formatFileSize` 处理文件服务与统计输出。

- **插件系统**
  - 插件中常用 `RuntimeUtil.sleep`、`RuntimeUtil.exec`、`RuntimeUtil.promiseEvent` 实现异步流程控制。
  - 使用 `RuntimeUtil.extractTextContent` 或 `String` 做调试输出与上下文处理。
...
- **[AgentRuntime 主类文档](agent-runtime.md)** - AgentRuntime 类的完整说明
