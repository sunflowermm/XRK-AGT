...
- **开发者在触发事件或监听器中必须提供**
  - `adapter`（小写）用于区分事件来源
  - `post_type`：`message`/`notice`/`request`/`device`/`command` 等
  - 细分类型：`message_type` / `notice_type` / `request_type` / `detail_type`、可选 `sub_type`
  - 标识字段：`user_id`（消息/请求类必需）、`group_id`（群事件必需）、`device_id`（设备事件必需）
  - 负载字段：`message`（标准消息段数组）或 `raw_message`
  - `time`：秒级时间戳（建议由事件源给出）
- **PluginsLoader 自动补全**
  - `self_id`：从 `device_id` / `stdin` / `Bot.uin[0]` 兜底推断
  - `bot`：只读挂载对应 Bot 实例
  - `event_id`：缺省则生成 `adapter_postType_timestamp_random`
  - 状态字段：`isDevice`/`isStdin`、基础 `sender.nickname/card`、`logText`
  - 工具与包装：统一的 `reply` 兜底、`getSendableMedia`、`throttle`、`getEventHistory`
...
```javascript
{
  ...
  // 类型标识（由OneBotEnhancer增强插件设置）
  isGroup: boolean,
  isPrivate: boolean,
  
  // 对象访问器（由OneBotEnhancer增强插件通过accept方法挂载）
  friend: FriendObject,    // 好友对象（延迟加载）
  group: GroupObject,      // 群对象（延迟加载）
  member: MemberObject,    // 成员对象（延迟加载）
  
  // @相关属性（由OneBotEnhancer增强插件处理）
...
3. 事件去重和标准化（基础字段）
   ↓
4. Bot.em 自动调用 Bot.prepareEvent（设置通用属性）
   ↓
5. 插件加载器处理 (PluginsLoader.deal)
   ↓
6. 初始化事件对象 (initEvent)
   ↓
7. 前置检查 (preCheck)
   ↓
8. 初始化插件列表 (initPlugins)
   ↓
9. 执行 accept 方法（每个插件，包括适配器增强插件）
   ↓
   - OneBotEnhancer: 挂载 friend/group/member/atBot 等OneBot特定属性
   - 其他插件: 执行自定义前置检查
   ↓
10. 处理上下文 (handleContext)
    ↓
11. 执行插件规则 (processRules)
    ↓
12. 调用插件方法
```
