## AdapterLoader 文档（src/infrastructure/adapter/loader.js）

`AdapterLoader` 负责从 `core/adapter` 目录动态加载各类消息适配器（如 QQ OneBotv11、企业微信等），并与 `Bot` 主类配合，为整个系统提供统一的消息入口。

---

## 职责与定位

- 扫描 `paths.coreAdapter`（即 `core/adapter`）目录中的所有 `.js` 文件。
- 使用 `import()` 动态载入适配器模块。
- 通过适配器内部代码将自身注册到：
  - `Bot.adapter`：适配器列表。
...
> 适配器文件通常不需要直接依赖 `AdapterLoader`，只要在模块内调用 `Bot.adapter.push(...)` 即可被框架识别。

---

## 关键属性

- `this.baseDir`：适配器所在目录，来自 `paths.coreAdapter`。
- `this.loggerNs`：日志命名空间，固定为 `'AdapterLoader'`。
...
1. 初始化统计对象 `summary`：
   ...
   - `failed`：导入失败数量。
   - `registered`：新注册的适配器数量（`bot.adapter.length` 的增量）。
...
4. 统计注册数量：
   - 假设适配器内部会向 `bot.adapter` 数组追加自身：
     - `summary.registered = bot.adapter.length - adapterCountBefore`。

5. 输出总结日志：
   - 类似：  
     `适配器加载完成: 成功X个, 注册Y个, 失败Z个`。
...
- **适配器文件（例如 `core/adapter/OneBotv11.js`）的典型结构：**
  - 在模块顶层执行：
...
- **消息流向：**
  ...
  2. `Bot.wsConnect` 根据路径选择对应的适配器处理函数。
  3. 适配器解析 JSON 上报，将其转换为统一事件结构（`post_type/message_type/...`）。
...
- **新增适配器**
  - 在 `core/adapter` 中新建 `XXX.js`。
  - 在文件内：
    - 通过 `Bot.adapter.push(new XXXAdapter())` 注册适配器。
    - 在 `load()` 中向 `Bot.wsf` 映射对应 WebSocket 路径。
    - 在 `message()` 中解析上报并调用 `Bot.em`。
  - 重启或通过相应命令触发适配器重载后，`AdapterLoader.load()` 会自动发现。

- **调试加载问题**
  - 查看启动日志中 `AdapterLoader` 名下的输出。
  - 若 `failed > 0`，可从 `summary.errors` 或控制台日志中找到对应错误。
  - 注意适配器文件必须是 ES Module（`export` 语法），并确保所有依赖可用。
