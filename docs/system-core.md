...
system-Core 提供了 **12 个** HTTP API 模块，覆盖系统管理的各个方面。
...
system-Core 工作流位于 **`core/system-Core/stream/*.js`**，由 **`AiStreamLoader`**（`src/infrastructure/ai-workflow/loader.js`）扫描 **`core/*/stream`** 自动加载。  
