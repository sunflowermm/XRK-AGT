...
| L1 架构 | [底层架构设计.md](底层架构设计.md)、[PROJECT_OVERVIEW.md](../PROJECT_OVERVIEW.md) | 分层边界、目录、特性概览 |
| L1 内置能力 | [system-core.md](system-core.md) | 11 HTTP / 7 stream / 15 plugin 等 API 与模块说明 |
...
| 项 | 标准值 | 维护位置 |
|----|--------|----------|
| HTTP API | 11 | `system-core.mjs`、`module-inventory` |
| stream | 7 | 同上 |
| plugin | 15 | 同上 |
| tasker | 4 | 同上 |
| events | 3 | 同上 |
| MCP 工具（仅七个自带工作流） | 68 | `core/system-Core/stream/*.js` 内 `registerMCPTool` |
| CI 单测（全量） | 63 pass | `package.json` → `pnpm test` |
| CI 单测（快速） | 58 pass | `pnpm test:fast` |
...
- [ ] README / PROJECT_OVERVIEW / system-core 无 9 stream、16 plugin、60/63 pass 等过期表述
- [ ] MCP：**68** 指 system-Core 七工作流；全仓库约 90 仅作日志参考且已标注
- [ ] 已本地执行 `pnpm test`、`pnpm test:e2e`，实测表日期与 pass 数已更新（若变更测试）
...
- [ ] 事件监听器基类为 **`EventListenerBase`**（`listener/base.js`），勿写旧 `EventListener`（`listener/listener.js`，加载器未使用）
- [ ] 插件 `rule[]` 无 `priority` 字段；顺序由 `super({ priority })` 控制（数字越小越先）
- [ ] Node 版本与 [node-26-runtime.md](node-26-runtime.md) 一致（≥26.0，无 node-fetch / 24.x 表述）
...
| 底层加载器/鉴权 | `底层架构设计.md`、`node-26-runtime.md`、`bot.md` / `server.md` | `框架测试指南` 若新增断言 |
| 部署方式 | `docker.md`、`README` 快速开始 | `PROJECT_OVERVIEW` |
| 仅文案/错别字 | 涉及页面即可 | — |
...
| 配置 / 渲染 | [config-base.md](config-base.md)、[renderer.md](renderer.md) |
