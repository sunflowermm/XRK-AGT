...
辅助工作流通过 `registerFunction` 或 `registerMCPTool` 注册功能，主工作流通过 `merge()` 方法合并：

```javascript
// 辅助工作流注册 Call Function（出现在 prompt 中）
this.registerFunction('save_knowledge', {
  prompt: () => `[保存知识:知识库名:内容] - 保存知识${getKnowledgePrompt()}`,
  handler: async (params, context) => { /* ... */ }
});

// 辅助工作流注册 MCP 工具（返回 JSON，不出现在 prompt 中）
...
**合并规则**：
- `registerFunction` 注册的 Call Function 会合并到主工作流的 `functions` Map
- `registerMCPTool` 注册的 MCP 工具会合并到主工作流的 `mcpTools` Map
- Call Function 的 `prompt` 会被收集到 `buildFunctionsPrompt()` 中
- MCP 工具不会出现在 prompt 中，但会注册到 MCP 服务器

#### 2.2 Prompt合并

- **buildSystemPrompt()**：只有主工作流的会被调用，构建系统提示词
- **buildFunctionsPrompt()**：收集所有已合并 Call Function 的 `prompt` 字段（MCP 工具不会出现在这里）
...
**Call Function**（出现在 prompt 中）：
- `[长期记忆:内容]` - 保存长期记忆
- `[删除记忆:序号]` - 删除记忆

**MCP 工具**（返回 JSON，不出现在 prompt 中）：
- `memory.query_memory` - 查询记忆（返回 JSON）
- `memory.list_memories` - 列出所有记忆（返回 JSON）
...
**MCP 工具**（返回 JSON，不出现在 prompt 中）：
- `database.query_knowledge` - 从知识库查询知识（返回 JSON）
- `database.list_knowledge` - 列出所有知识库（返回 JSON）
...
#### `registerFunction(name, options)` - 注册 Call Function

注册 Call Function（供 AI 内部调用），支持动态prompt：

```javascript
this.registerFunction('my_func', {
  prompt: () => `动态生成的prompt`,  // 支持函数类型，会出现在 AI prompt 中
  parser: (text, context) => {
    // 解析文本中的命令格式
    const match = text.match(/\[我的功能:([^\]]+)\]/);
    if (!match) {
    ...
    return {
      functions: [{ type: 'my_func', params: { param: match[1] } }],
      cleanText: text.replace(/\[我的功能:[^\]]+\]/g, '').trim()
    };
  },
  handler: async (params, context) => {
...
#### `registerMCPTool(name, options)` - 注册 MCP 工具

注册 MCP 工具（供外部系统调用），返回 JSON 格式结果：
...
5. **可扩展性**：
   - 基类不再硬编码特定工作流名称，支持动态扩展
   - 工作流合并机制支持任意工作流，不限于特定名称
   - LLM提供商配置支持动态扩展，无需修改基类代码
