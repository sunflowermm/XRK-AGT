...
**重要**：辅助工作流的 `buildSystemPrompt()` 在合并时不会被调用，只有函数的 `prompt` 字段会被合并。
...
```
1. 用户请求 → 核心工作流
2. 构建Prompt（buildSystemPrompt + buildFunctionsPrompt）
3. AI决策 → 输出命令（如 [回桌面]）
4. 解析命令 → 执行函数
5. 返回结果 → 用户
```
...
1. 创建新的核心工作流文件
2. 实现 `buildSystemPrompt()` 和 `buildFunctionsPrompt()`
