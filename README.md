...
# 代理配置（容器/本机访问外网 LLM API 等，可选；**不含**内置模型下载）
...
框架 CI **只统计 `core/system-Core/` 官方模块**（其他 core 不计入达标线；vendor = 未 git 入库的本地插件）。标准值源码：`tests/helpers/system-core.mjs`。运行测试需 **Node ≥ 26**（见 `package.json` → `engines.node`）。
