...
class ListenerLoader {
  ...
  async load() {
    // 步骤1: 加载监听事件
    global.Bot.makeLog('info', "加载监听事件中...", 'ListenerLoader');
    let eventCount = 0
    ...
    try {
      ...
      try {
        ...
        global.Bot.makeLog('warn', `事件目录不存在: ${eventsDir}，跳过加载`, 'ListenerLoader');
        global.Bot.makeLog('info', `加载监听事件[${eventCount}个]`, 'ListenerLoader');
      ...
      for (const file of eventFiles) {
        global.Bot.makeLog('debug', `加载监听事件: ${file}`, 'ListenerLoader');
        try {
          ...
          global.Bot.makeLog('error', `监听事件加载错误: ${file}`, 'ListenerLoader', err);
        }
      }
    } catch (error) {
      global.Bot.makeLog('error', "加载事件目录失败", 'ListenerLoader', error);
      throw error;
    }
  ...
  async loadAdapters() {
      ...
      // 2.2: 初始化已注册的适配器
      global.Bot.makeLog('info', "初始化适配器中...", 'ListenerLoader');
      let adapterCount = 0
      let adapterErrorCount = 0
      
      if (!Bot.adapter || Bot.adapter.length === 0) {
        global.Bot.makeLog('warn', "未找到已注册的适配器", 'ListenerLoader');
      } else {
        for (const adapter of Bot.adapter) {
          try {
            if (!adapter || typeof adapter.load !== 'function') {
              global.Bot.makeLog('warn', `适配器无效: ${adapter?.name || 'unknown'}(${adapter?.id || 'unknown'})`, 'ListenerLoader');
              continue
            }
            
            global.Bot.makeLog('debug', `初始化适配器: ${adapter.name}(${adapter.id})`, 'ListenerLoader');
            await adapter.load()
            adapterCount++
          } catch (err) {
            global.Bot.makeLog('error', `适配器初始化错误: ${adapter?.name || 'unknown'}(${adapter?.id || 'unknown'})`, 'ListenerLoader', err)
            adapterErrorCount++
          }
        }
      }
      
      global.Bot.makeLog('info', `加载适配器[${adapterCount}个]${adapterErrorCount > 0 ? `, 失败${adapterErrorCount}个` : ''}`, 'ListenerLoader');
  }
}
